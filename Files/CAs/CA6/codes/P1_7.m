...
fc = 5;
phi1 = 2 * pi * fc * t + (2 * pi * V / R1) * t;
phi2 = 2 * pi * fc * t + (2 * pi * V / R2) * t;
...
estimated_V = V; 
estimated_R1 = (V * t(end)) / ((phi1(end) - phi1(1)) / (2 * pi * fc));
estimated_R2 = (V * t(end)) / ((phi2(end) - phi2(1)) / (2 * pi * fc));

disp(['Estimated V: ', num2str(estimated_V), ' m/s']);
