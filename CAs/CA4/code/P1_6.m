...
estimated_V1 = (R1 / (2 * pi * t(end))) * ((phi1(end) - phi1(1)) / t(end));
estimated_R1 = (V1 * t(end)) / ((phi1(end) - phi1(1)) / (2 * pi * fc));

estimated_V2 = (R2 / (2 * pi * t(end))) * ((phi2(end) - phi2(1)) / t(end));
estimated_R2 = (V2 * t(end)) / ((phi2(end) - phi2(1)) / (2 * pi * fc));

disp(['Estimated V1: ', num2str(estimated_V1), ' m/s']);
disp(['Estimated R1: ', num2str(estimated_R1), ' m']);
disp(['Estimated V2: ', num2str(estimated_V2), ' m/s']);
disp(['Estimated R2: ', num2str(estimated_R2), ' m']);
